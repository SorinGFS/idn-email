...
Continuous integration runs this suite on Node.js 24.13.1 and 26 across Ubuntu, Windows, and macOS. CI checks out the public test concern and the `gh-workspace-data v0.4.1` version-layer helper explicitly.
...
This release declares Node.js `>=24.13.1 <25 || >=26.0.0`. The range matches the Unicode-data requirement of the selected `idn-hostname` line and makes the package's direct runtime contract visible to installers and tooling.
