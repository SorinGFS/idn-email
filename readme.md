...
The selected `idn-hostname` release targets Unicode 15.1.0. `idn-email` is CommonJS and supports these runtime options:

- **Node.js:** `>=20.12.0 <21 || >=22.0.0`, matching the Unicode-data requirement of `idn-hostname@15.1`.
- **Browsers:** an [ECMAScript 2022](https://262.ecma-international.org/13.0/) runtime with Unicode 15.1 normalization/property data and a CommonJS-capable bundler.
...
The Unicode 15.1 address suite contains 54 independently reported tests: 41 numeric fixtures plus 13 shared API and conformance scenarios. Numeric fixtures run through `isIdnEmailAddress`.
...
Continuous integration runs the address suite on Node.js 20.12.0, 22, 24, and 26 across Ubuntu, Windows, and macOS. CI checks out the public test concern and the `gh-workspace-data v0.5.0` traversal runtime explicitly.
...
The `15.1.x` release line selects Unicode 15.1.0 through `idn-hostname@15.1.x`. Its Node.js engine range and browser runtime requirements are stated above under runtime support. This is the package's first Unicode-versioned release line, so there is no preceding release line to compare.
