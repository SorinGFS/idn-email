...
The selected `idn-hostname` release targets Unicode 16.0.0. `idn-email` is CommonJS and supports these runtime options:

- **Node.js:** `>=22.12.0 <23 || >=24.0.0`, matching the Unicode-data requirement of `idn-hostname@16`.
- **Browsers:** an [ECMAScript 2022](https://262.ecma-international.org/13.0/) runtime with Unicode 16 normalization/property data and a CommonJS-capable bundler.
...
The Unicode 16 address suite contains 61 independently reported tests: 48 cumulative numeric fixtures (41 from Unicode 15.1 and seven from Unicode 16.0) plus 13 shared API and conformance scenarios. Numeric fixtures run through `isIdnEmailAddress`.
...
The `16.0.x` release line selects Unicode 16.0.0 through `idn-hostname@16.0.x`. Its Node.js engine range and browser runtime requirements are stated above under runtime support. Unicode 16 expands the accepted hostname repertoire relative to `15.1.x`, but it also corrects the properties of U+1171E AHOM CONSONANT SIGN MEDIAL RA from `Bidi_Class=NSM` and `Joining_Type=T` to `Bidi_Class=L` and `Joining_Type=U`. Addresses whose hostname validity depends on the earlier properties can therefore become invalid under RFC 5893 Bidi or RFC 5892 CONTEXTJ validation.
