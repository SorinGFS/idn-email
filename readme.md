...
The Unicode 16.0 release line runs 48 independently reported package fixtures: 41 inherited Unicode 15.1 fixtures and seven Unicode 16 additions. Numeric fixtures remain in delta-only version layers and accumulate because `#/public/tests/index.json` marks `isIdnEmail` backwards compatible.
...
This release declares Node.js `>=22.12.0 <23 || >=24.0.0`. The range matches the Unicode-data requirement of the selected `idn-hostname` line and makes the package's direct runtime contract visible to installers and tooling.
